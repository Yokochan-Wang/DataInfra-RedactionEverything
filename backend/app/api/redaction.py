...
@router.post("/redaction/{file_id}/restore-categories", response_model=RestoreCategoriesResult)
async def restore_redaction_categories(
    file_id: str,
    request: RestoreCategoriesRequest,
    owner_id: str = Depends(require_auth),
):
    """Generate a copy with selected redaction categories restored."""
    try:
        _fms.assert_file_owner(file_id, owner_id)
        response = await _orch.restore_categories(file_id, request.entity_types)
    except ValueError as exc:
        detail = str(exc)
        status = 404 if detail in {"file not found", "文件不存在"} else 400
    ...
    audit_log(
        ...
        "file",
        file_id,
        detail={"categories": response.restored_categories, "restore_id": response.restore_id},
    )
    return response


@router.get("/redaction/{file_id}/restore/{restore_id}/download")
async def download_restored_file(
    ...
    restored = next(
        (
            item
            for item in snapshot.get("restoration_history", [])
            if isinstance(item, dict) and item.get("restore_id") == restore_id
        ),
        None,
    )
    output_path = restored.get("output_path") if restored else None
    if not isinstance(output_path, str) or not output_path:
        raise HTTPException(status_code=404, detail="还原文件不存在")
    if not _fms.safe_path_in_dir(output_path, settings.OUTPUT_DIR) or not os.path.isfile(output_path):
        raise HTTPException(status_code=404, detail="还原文件不存在")

    return FileResponse(
        path=output_path,
        filename=f"restored_{snapshot.get('original_filename') or file_id}",
