...
class RestoreCategoriesRequest(BaseModel):
    """Selectively restore redaction categories in a previously redacted file."""

    entity_types: list[str] = Field(..., min_length=1, description="Entity type ids to restore")


class RestoreCategoriesResult(BaseModel):
    file_id: str
    restore_id: str
    restored_categories: list[str] = Field(default_factory=list)
    retained_redaction_categories: list[str] = Field(default_factory=list)
    restored_entity_count: int = 0
    retained_redaction_count: int = 0
    output_filename: str
    download_url: str
    output_path: str | None = Field(default=None, exclude=True)
