...
class Settings(BaseSettings):
    ...
    # Master switch for the whole LocateAnything chain. When off the visual
    # features stage is skipped entirely (see redaction_orchestrator.detect_vision)
    # instead of timing out against a model server that is not running.
    LOCATE_ANYTHING_ENABLED: bool = True
    ...
    HAS_TEXT_EXTERNAL_BASE_URL: str = ""
    HAS_TEXT_API_KEY: str = ""
    ...
    # 脱敏可逆性加固：脱敏后清除 file_store 中的 content/pages/entities/entity_map，
    # 占位符不再暴露实体类型与数量，并追加残余敏感值清理（表格/页眉页脚等漏脱敏）。
    PRIVACY_HARDENING_ENABLED: bool = False
    # 脱敏成功后立即删除 uploads 下的原始文件（旧开关，默认关闭；建议改用冷存储）。
    PURGE_ORIGINAL_UPLOADS_AFTER_REDACTION: bool = False
    # 原始文件在脱敏完成后的热保留时长（分钟）。到期后加密移入冷存储。
    # 0 = 立即归档；-1 = 禁用自动归档。
    ORIGINAL_RETENTION_MINUTES: int = 1440
    # 加密冷存储目录；空 = DATA_DIR/cold_storage。
    ...
    model_config = SettingsConfigDict(
        # 根目录 .env 是唯一配置入口（后读取者覆盖），backend/.env 仅作兼容
...
def get_has_chat_base_url() -> str:
    ...
    if s.HAS_TEXT_RUNTIME.strip().lower() == "external":
        ...
        if external_url:
            return _resolve_wsl_localhost_url(external_url)
