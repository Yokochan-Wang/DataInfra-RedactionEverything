...
async def lifespan(app: FastAPI):
    ...
    from app.services.retention_service import cold_archive_loop, retention_loop
    _retention_task = asyncio.create_task(retention_loop())
    _cold_archive_task = asyncio.create_task(cold_archive_loop())
...
async def services_health():
    ...
    locate_anything_enabled = os.environ.get("LOCATE_ANYTHING_ENABLED", "1").lower() not in {
        "0",
        "false",
        "no",
        "off",
    }
    required_services = ["paddle_ocr", "has_ner"]
    if locate_anything_enabled:
        ...
        required_services.append("visual_features")
    all_online = all(services[key]["status"] == "online" for key in required_services)
