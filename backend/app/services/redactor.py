...
from app.services.redaction.residual_sanitizer import build_residual_replacements
...
class Redactor(TextRedactorMixin, ImageRedactorMixin):
    ...
    async def redact(
        ...
        residual_replacements: dict[str, str] = {}
        if getattr(settings, "PRIVACY_HARDENING_ENABLED", False):
            residual_replacements = build_residual_replacements(file_info.get("content") or "")
    ...
    def _ensure_original_file_available(self, file_info: dict) -> str | None:
        file_path = self._resolve_existing_path(file_info.get("file_path"), settings.UPLOAD_DIR) or file_info.get("file_path")
        if isinstance(file_path, str) and os.path.exists(file_path):
            return file_path
        if not isinstance(file_path, str) or not file_info.get("cold_storage_path"):
            return file_path
        from app.services.cold_storage import restore_metadata, restore_original_from_cold

        file_id = file_info.get("id") or file_info.get("file_id")
        restore_original_from_cold(str(file_id), file_path)
        if not file_info.get("entity_map"):
            metadata = restore_metadata(str(file_id))
            file_info["entity_map"] = metadata.get("entity_map") or {}
            file_info.setdefault("entities", metadata.get("entities") or [])
        return file_path
    ...
    async def get_comparison(self, file_info: dict) -> dict:
        ...
        """
        file_type = file_info["file_type"]
        original_path = self._ensure_original_file_available(file_info)
