...
class VisionService:
    ...
    async def _detect_with_visual_features(
        ...
        # LOCATE_ANYTHING_ENABLED=0 only skips the grounding calls themselves:
        # OCR-fallback categories (seals) and the local cv2 machine-code pass
        # below keep working, so turning the model off costs nothing else.
        la_enabled = bool(getattr(settings, "LOCATE_ANYTHING_ENABLED", True))
        if la_enabled:
            ...
            locate_boxes, stage_duration_ms = [], {}
            logger.info("LocateAnything disabled; skipping visual grounding stage")
            self.last_warnings.append("LocateAnything 未启用，视觉 grounding 阶段已跳过")
        checklist_boxes: list[BoundingBox] = []
        checklist_duration_ms: dict[str, int] = {}
        if checklist_types and la_enabled:
            try:
