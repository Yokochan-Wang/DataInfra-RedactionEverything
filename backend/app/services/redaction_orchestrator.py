...
_RAW_REDACTION_FIELDS = ("content", "pages")


def _sanitize_redacted_record(info: dict[str, Any]) -> None:
    if not getattr(settings, "PRIVACY_HARDENING_ENABLED", False):
        return
    for field in _RAW_REDACTION_FIELDS:
        info.pop(field, None)
    info["original_data_purged"] = True


def _ensure_original_file_available(file_id: str, info: dict[str, Any]) -> None:
    ...
    if not file_path_exists:
        if not isinstance(file_path, str) or not info.get("cold_storage_path"):
            raise ValueError("原始文件已按保留策略清除，无法还原")
        from app.services.cold_storage import restore_original_from_cold

        restore_original_from_cold(file_id, file_path)

    if not info.get("entities") or not info.get("entity_map"):
        if info.get("cold_storage_path"):
            from app.services.cold_storage import restore_metadata

            metadata = restore_metadata(file_id)
            info.setdefault("entities", metadata.get("entities") or [])
            info.setdefault("entity_map", metadata.get("entity_map") or {})


def _purge_original_upload(info: dict[str, Any]) -> None:
    if not getattr(settings, "PURGE_ORIGINAL_UPLOADS_AFTER_REDACTION", False):
        return
    file_path = info.get("file_path")
    ...
    if not safe_path_in_dir(file_path, settings.UPLOAD_DIR) or not os.path.isfile(file_path):
        return
    try:
        os.remove(file_path)
        info["original_file_purged"] = True
    except OSError:
        logger.warning("unable to purge original upload after redaction: %s", file_path, exc_info=True)
...
async def execute_redaction(request: RedactionRequest) -> RedactionResult:
    ...
    privacy_hardening = bool(getattr(settings, "PRIVACY_HARDENING_ENABLED", False))
    ...
    async with lock:
        ...
        if info is None:
            ...
            info["redaction_completed_at"] = datetime.now(UTC).isoformat()
...
async def restore_categories(file_id: str, requested_types: list[str]) -> RestoreCategoriesResult:
    """Regenerate a redacted file with selected entity categories restored."""
    ...
    lock = _get_file_store_lock()
    async with lock:
        snapshot = dict(file_store.get(file_id) or {})
    if not snapshot:
        raise ValueError("file not found")
    if not snapshot.get("output_path"):
        raise ValueError("文件尚未匿名化，无法还原")

    output_path = snapshot.get("output_path")
    if not isinstance(output_path, str) or not safe_path_in_dir(output_path, settings.OUTPUT_DIR) or not os.path.isfile(output_path):
        raise ValueError("匿名化产物不存在，无法还原")

    _ensure_original_file_available(file_id, snapshot)

    restore_types = {
        canonical_type_id(str(entity_type))
        for entity_type in requested_types
        if canonical_type_id(str(entity_type))
    }
    if not restore_types:
        raise ValueError("没有有效的还原类别")
    ...
    for raw_entity in snapshot.get("entities") or []:
        if isinstance(raw_entity, Entity):
            ...
            entities.append(Entity.model_validate(raw_entity))

    boxes: list[BoundingBox] = []
    raw_boxes = snapshot.get("bounding_boxes") or {}
    if isinstance(raw_boxes, dict):
        for page_boxes in raw_boxes.values():
            if not isinstance(page_boxes, list):
                continue
            for raw_box in page_boxes:
                boxes.append(raw_box if isinstance(raw_box, BoundingBox) else BoundingBox.model_validate(raw_box))
    elif isinstance(raw_boxes, list):
        boxes.extend(
            raw_box if isinstance(raw_box, BoundingBox) else BoundingBox.model_validate(raw_box)
            for raw_box in raw_boxes
        )

    def was_selected(item: Entity | BoundingBox) -> bool:
        return getattr(item, "selected", True) is not False

    def item_type(item: Entity | BoundingBox) -> str:
        return canonical_type_id(str(getattr(item, "type", "")))

    selected_before = [item for item in [*entities, *boxes] if was_selected(item)]
    available_types = {item_type(item) for item in selected_before}
    matched_restore_types = sorted(restore_types & available_types)
    if not matched_restore_types:
        raise ValueError("所选类别不在该文件的匿名化范围内")

    restored_count = 0
    retained_count = 0
    entity_map = dict(snapshot.get("entity_map") or {})
    custom_replacements: dict[str, str] = {}

    for entity in entities:
        should_restore = item_type(entity) in restore_types
        next_selected = was_selected(entity) and not should_restore
        if was_selected(entity) and should_restore:
            restored_count += 1
        elif next_selected:
            retained_count += 1
            if entity.text in entity_map:
                custom_replacements[entity.text] = entity_map[entity.text]
        entity.selected = next_selected

    for box in boxes:
        should_restore = item_type(box) in restore_types
        next_selected = was_selected(box) and not should_restore
        if was_selected(box) and should_restore:
            restored_count += 1
        elif next_selected:
            retained_count += 1
        box.selected = next_selected

    config = RedactionConfig(
        replacement_mode=ReplacementMode.CUSTOM,
    ...
    restore_id = str(result["output_file_id"])
    retained_types = sorted(available_types - set(matched_restore_types))
    history_entry = {
        "restore_id": restore_id,
        "restored_categories": matched_restore_types,
        "retained_redaction_categories": retained_types,
        "restored_entity_count": restored_count,
        "retained_redaction_count": retained_count,
        "output_path": result.get("output_path"),
        "created_at": datetime.now(UTC).isoformat(),
    }
    async with lock:
        current = file_store.get(file_id)
        if current is None:
            raise ValueError("file not found")
        current.setdefault("restoration_history", []).append(history_entry)
        file_store.set(file_id, current)
    ...
    return RestoreCategoriesResult(
        ...
        restored_entity_count=restored_count,
        retained_redaction_count=retained_count,
        output_filename=f"restored_{snapshot.get('original_filename') or file_id}",
        download_url=f"/api/v1/redaction/{file_id}/restore/{restore_id}/download",
        output_path=result.get("output_path"),
    )
